...
def start_agent():
    ...
    if res.returncode != 0:
        ...
        # We MUST spawn an interactive bash shell so the user's ~/.bashrc aliases (like 'aim') are loaded
        subprocess.run(["tmux", "new-session", "-d", "-s", TMUX_SESSION, "bash", "-ic", "gemini -m gemini-3-flash -y"], cwd=TARGET_DIR, check=True)
