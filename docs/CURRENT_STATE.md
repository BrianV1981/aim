...
- **Advanced Forensic Search:** Upgraded `aim search` with `--context`, `--full`, and `--session` filters. Agents can now perform targeted deep-dives into specific sessions with 2000+ characters of surrounding text, significantly improving forensic accuracy.
