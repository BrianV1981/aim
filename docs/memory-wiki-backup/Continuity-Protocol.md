...
- **Deprecated ROADMAP.md:** Agents must rely exclusively on `continuity/ISSUE_TRACKER.md` as the source of truth for task state; static `.md` roadmaps frequently lead to hallucinations and architectural drift.
- **Legacy Fallback Tails Disabled:** `FALLBACK_TAIL.md` logic has been eradicated to prevent redundant context from bloating new tickets during handoffs.
