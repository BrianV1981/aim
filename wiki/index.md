# A.I.M. Wiki Index

## 🧠 Core Architecture
- [Feature: GitOps Bridge](Feature-GitOps-Bridge.md) - Atomic deployment and initialization protocols.
- [Continuity Protocol](Continuity-Protocol.md) - Handoffs and reincarnation logic.
- [Layered Engram Architecture](Layered-Engram-Architecture.md) - Data persistence and retrieval design.

## ⚙️ Operations & Configuration
- [Configuration](configuration.md) - Model locking, environment variables, and state persistence.
- [Model-Lock Protocol](Model-Lock-Protocol.md) - Defensive configuration against model downgrades.
- [Rate Limits and Performance](rate_limits_and_performance.md) - RPM/TPM management and 429 error mitigation.
- [Development Standards](development_standards.md) - Scripting, pathing, and execution requirements.
- [Best Practices](best_practices.md) - Reliability patterns and professional standards.

## 🚨 Troubleshooting & Issues
- [Critical Failures](Critical-Failures.md) - Error handling, rate limits, and transparency logic.
- [Issue Tracking](issue_tracking.md) - Active critical failures and infrastructure snags.
- [Issue Archive](Issue-Archive.md) - Historical record of resolved issues.
- [Diagnostics](diagnostics.md) - System health checks and recovery protocols.

## 📖 Intelligence & Meta
- [Technical Intelligence](Technical-Intelligence.md) - Platform-specific findings and account identification.
- [GitHub Workflow Pattern](GitHub-Workflow-Pattern.md) - Standardized GitOps and PR management.
- [Wiki Schema](WIKI_SCHEMA.md) - Standards for this knowledge base.
- [Log](log.md) - Chronological record of wiki updates.
