...
Automatically generated by the **Flash Distiller** on session exit. The Pulse captures the "Mental Model" and "Active Edge" of the workspace. It is injected into the *very next* session by the `context_injector` to ensure zero-latency continuity. (Manual override available via the `/handoff` command).
