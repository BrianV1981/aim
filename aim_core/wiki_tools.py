...
def process_wiki():
    ...
    prompt = f"Wake up. 1. IMMEDIATELY read `AGENT.md` in this directory to understand your core Subconscious Daemon directives. 2. You have new session chunks waiting in the `_ingest/` directory. You MUST process them methodically ONE BY ONE: 3. Read the first chunk. 4. Weave its architectural insights into `index.md`, `log.md`, or relevant concept pages. 5. Immediately DELETE that specific chunk from `_ingest/` before moving to the next. 6. Repeat until the `_ingest/` directory is completely empty. 7. Once empty, use run_shell_command to execute: `cd {base_dir} && git add memory-wiki/ && git commit -m \"Chore(Subconscious): Synthesize and weave session lore\"`. 8. Finally, execute: `tmux kill-session -t {session_name}` to cleanly terminate this container."
